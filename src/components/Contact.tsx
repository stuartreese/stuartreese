    ...
    <section id="contact" className="scroll-mt-24 py-20 sm:py-28">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <SectionHeading
          ...
          title={
            <>
              Would love to tell you more <span className="display-italic text-forest">if you have time.</span>
            </>
          }
        ...
        <div className="mt-12 grid gap-8 lg:grid-cols-2">
          <Reveal className="rounded-3xl border border-sand bg-cream p-6 shadow-soft sm:p-8">
            <form onSubmit={onSubmit} className="space-y-4" noValidate={false}>
              ...
              <div className="flex flex-wrap items-center gap-3 pt-2">
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-full bg-clay px-5 py-3 text-sm font-medium text-cream shadow-soft transition hover:bg-ember hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-70 disabled:hover:translate-y-0"
                >
              ...
              <p aria-live="polite" className="min-h-6 text-sm">
                {status === "sent" && <span className="text-forest">Thanks! Your note is on its way. I will get back to you soon.</span>}
          ...
          <Reveal delay={100} className="overflow-hidden rounded-3xl border border-sand bg-cream shadow-soft">
            <div className="flex items-center justify-between border-b border-sand px-5 py-3">
              ...
              <a href={site.links.calendly} target="_blank" rel="noreferrer" className="text-sm text-clay hover:text-ember">
                Open in Calendly ↗
              </a>
            </div>
            <iframe
              title="Book a 30 minute chat with Stuart"
              src={`${site.links.calendly}?hide_gdpr_banner=1&background_color=f6f1e6&text_color=1f2a24&primary_color=1f4a3a`}
...
function Field({ label, name, type = "text", as = "input", required, autoComplete }: FieldProps) {
  const cls =
    "w-full rounded-2xl border border-sand bg-parchment/40 px-4 py-3 text-base outline-none transition placeholder:text-ink/40 focus:border-forest focus:bg-cream focus:ring-2 focus:ring-forest/20";
  return (
